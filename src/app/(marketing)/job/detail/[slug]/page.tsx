...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!job || isInternshipListing(job)) return { title: "Job | CareerVerse AI" };
  return {
...
export default async function JobDetailSlugPage({ params }: Props) {
  const { slug } = await params;
  const job = await resolveListingBySlug(slug);
  if (!job || isInternshipListing(job)) notFound();
  return <OpeningDetail job={job} kind="job" />;
}
