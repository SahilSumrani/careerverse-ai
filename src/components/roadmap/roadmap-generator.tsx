...
export function RoadmapGenerator({
  ...
  useEffect(() => {
    ...
    const fromQuery =
      initialTitle && careers.find((c) => c.title.toLowerCase() === initialTitle.toLowerCase());
    if (fromQuery) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="cv-shell-inner">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
                  Stages come from your CareerVerse profile and AI — not canned demo paths.
                </p>
              </div>
          ...
          <div className="space-y-5 p-5 md:p-6">
              ...
              <div>
                ...
                <div className="cv-chip-row">
                  {suggested.map((title) => {
                    ...
                    const active = match
                      ? roleId === match.id
                      : roleId === "custom" && customTitle === title;
                    return (
                      <button
                        key={title}
                        type="button"
            ...
            <div>
              ...
              {careers.length ? (
                <div className="cv-stagger grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {careers.map((r) => (
                    <button
                      key={r.id}
                      type="button"
                      onClick={() => setRoleId(r.id)}
                      data-active={roleId === r.id}
                      className="cv-tile cv-panel-interactive p-3.5 text-left"
                    >
      ...
      {stages ? (
        <div className="cv-stagger space-y-3">
          {stages.map((stage, index) => {
            ...
            return (
              <div key={stage.key} className="cv-panel overflow-hidden p-0">
                <button
                  type="button"
                    ...
                    {stage.items.map((item, i) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => toggle(id)}
                          data-active={checked}
                          className="cv-tile flex w-full items-start gap-3 px-3 py-3 text-left"
                        >
                          <span
                            className={cn(
      ...
      {stages ? (
        <div className="cv-panel p-5">
          <h3 className="text-[15px] font-semibold tracking-tight">About this plan</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            Personalized with your skills, gaps, and goals. Not a guarantee of hiring outcomes — regenerate
            anytime.
          </p>
        </div>
      ) : null}
